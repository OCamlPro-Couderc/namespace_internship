...
transformer l'en-tête en un ensemble de structure pour utiliser explicitement le
...
\begin{verbatim}
ocamlc -c -apply impl.cmi my_module.cmo
\end{verbatim}
...
\chapter{Contexte, exemples d'utilisation et analyse}

A découper en deux chapitres ?

\section{Environnement de travail}

!!!A réécrire!!!

\subsection{Implémentation de namespaces}
...
\subsection{Big functors}

L'ajout des functor units demande en revanche de comprendre un patch qui n'a
jamais été intégré : il faut comprendre son fonctionnement, analyser les
différences avec le code actuel (puisqu'il était destiné à une ancienne version)
mais aussi comment l'intégrer et le réutiliser intelligement.

Les functor-units ne touchent pratiquement pas à l'environnement de typage ou au
typeur lui-même : les changements se font principalement au niveau du format des
unités de compilation et de la génération de code. Le challenge est ici de
comprendre comment le code est généré (pour réaliser les unité functorisées), et
comment utiliser ce code pour créer les instanciations. L'avantage du
compilateur est qu'il possède un même langage intermédiaire pour les deux
\emph{back-end} (bytecode et assembleur) : le lambda. L'idée est donc de générer
un même lambda (ou du moins réutiliser celui-ci) des deux côtés de la même
manière. Cependant, le lambda généré est légèrement différent et demande donc
des adaptations.
...
choix de mot-clés qui sont déjà utilisés dans le langage. 
...
<ns_conf> ::= <mod_longident> `.' `(' <modules_constraints> `)'
